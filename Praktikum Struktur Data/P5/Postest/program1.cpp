...
class KTP{
    ...
    friend std::ostream &operator<<(std::ostream &keluaran, KTP &data){
        ...
        //     break;
        // }
        keluaran<<data.getNama()<<" "<<data.getNik();
        return keluaran;
    }
    ...
    public:
        ...
        KTP(const char *nama, const char *nik){
            this->pengguna.nama = nama;
            this->pengguna.nik = nik;
        }

        void setNamaDanNik(const char *nama, const char *nik){
            this->pengguna.nama = nama;
            this->pengguna.nik = nik;
        }

        void cetak(){
            std::cout<<"Nama : "<<pengguna.nama<<std::endl;
            std::cout<<"NIK  : "<<pengguna.nik<<std::endl;
        }
...
Node<KTP> *head, *tail, *jalan, *hapus;
...
class DoublyLinkedList{
    public:
        ...
        void removeAt(int posisi){
            Node<T> *temp = new Node<T>();
            int n = 0;
            if(posisi - 2 < 0 || posisi > getLen()){
            ...
            hapus = jalan->next;
            hapus->prev = 0;
            hapus->next->prev = jalan;
            jalan->next = hapus->next;
            hapus->next = 0;
            delete hapus;
    ...
    while(pilih != '4'){
        ...
        switch(pilih){
            case '1':
                ...
                std::cin>>nik;
                buffer.setNamaDanNik(nama.c_str(), nik.c_str());
                list.tambah(buffer);
                break;
            case '2':
                list.remove();
