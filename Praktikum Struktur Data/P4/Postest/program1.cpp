...
class Mahasiswa{
    friend std::ostream &operator<<(std::ostream &out, Mahasiswa &obj){
        out<<obj.nama<<" "<<obj.kelas<<" "<<obj.nim;
        return out;
    }
    public:
        Mahasiswa(){}
        void set(const char *nama, int nim, char kelas){
            this->nama = nama;
            this->nim = nim;
            this->kelas = kelas;
        }
...
Node<Mahasiswa> *head, *tail, *jalan, *hapus;
...
int main(){
    Mahasiswa mhs1;
    DoublyLinkedList<Mahasiswa> list1;
    mhs1.set("Fajar", 165, 'C');    
    list1.tambah(mhs1);
    list1.cetak();
    return 0;
}